...
router.get('/config', verify ,async (req,res)=> {
 ...
 router.post('/config', verify ,async (req,res)=> {
    ...
    var myobject = {"name": req.body.name, "email": req.body.email, "password": req.body.password}
    ...
    try {   
        ...
        const modifyUserData = await User.update({'_id': req.id}, {$set: {name: req.body.name, email : req.body.email , password : hashedPassword}}, function(err, result){
            console.log(result);
        });

...
router.get('/jsonget', verify, async (req, res) => {
  ...
  var query = await Client.find({}, { English: 0, Español: 0, Français: 0, __v: 0 })
...
router.get('/jsonok/:type', verify, async (req, res) => {
  ...
  var a = req.query;
  var respnseAddID = "";
  var okResponse = {"_id":1, "fullname": "", "name":"Name modified by server", "surname":"", "father":"", "mother":"", "sex":"", "placeofbirthlocal":"", "dateofbirth":"","noregistry":""};
  if(methode == "type=PUT")
  {
    console.log("PUT");
  ...
  let data = {"_id":sendId, "fullname": a.fullname, "s0":{ "name":a['s0.name'], "surname":a['s0.surname'], 
  "father": a['s0.father'], "mother": a['s0.mother'], "sex": a['s0.sex'], 
  "placeofbirthlocal": a['s0.placeofbirthlocal'], "dateofbirth":  a['s0.dateofbirth'],"noregistry": a['s0.noregistry'] }}
  ...
  try {
    ...
    if (
      ...
      if (fs.existsSync(path) && fs.existsSync(docxPath)) {
        ...
        var outputPath = GenerateDocx(data, docxPath, docArray, req.query.doc,clientName)

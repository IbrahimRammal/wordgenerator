$(document).ready(function () {
    ...
    $.ajax({
        url: "/api/invoice/invoiceedit/GetData?action=sworn&id=2",
        type: "POST",
        dataType: "json",
        cache: true,
        //data: { },
    ...
    var data = new ej.data.DataManager({
        ...
        insertUrl: hostUrl + "invoiceedit/GetData?action=sworn&id=2",
        updateUrl: hostUrl + "invoiceedit/GetData?action=sworn&id=2",
        removeUrl: hostUrl + "invoiceedit/GetData?action=sworn&id=2",
    });
      ...
      var grid = new ej.grids.Grid({
        ...
        if(keys.length == 5 && keys[3] != 'undefined' && typeof keys[3] != 'undefined' && keys[3] != '' )
        {
          ...
          $.ajax({
            ...
            dataType: "json",
            cache: true,
            data: { language: "English", invoice: keys[2], ClientID: keys[0],currency: args.data.currency, total: args.data.total, invoiceSchemaID: keys[1], transcation: keys[4], invoiceNumber: args.data.invoiceNumber, docModel: "SwornReceiptVoucher", mode: "invoice", docID: keys[3], id: keys[0] },
            success: function (fixtures) {
                var html = fixtures["html"];
                // var paid = fixtures["paid"];
                $("#panel1").html(html);
          ...
          $.ajax({
            ...
            dataType: "json",
            cache: true,
            data: { language: "English", invoice: keys[2], ClientID: keys[0],currency: args.data.currency, total: args.data.total, invoiceSchemaID: keys[1], transcation: keys[4], invoiceNumber: args.data.invoiceNumber, type: "Sworn Receipt Voucher", mode: "invoice", docID: "", id: keys[0] },
            success: function (fixtures) {
                var html = fixtures["html"];
                // var paid = fixtures["paid"];
                $("#panel1").html(html);

$(document).ready(function () {
    ...
    $.ajax({
        url: "/api/invoice/invoiceedit/GetData?action=purchase&id=1",
        type: "POST",
        dataType: "json",
        cache: true,
        //data: { },
    ...
    var data = new ej.data.DataManager({
        ...
        insertUrl: hostUrl + "invoiceedit/GetData?action=purchase&id=1",
        updateUrl: hostUrl + "invoiceedit/GetData?action=purchase&id=1",
        removeUrl: hostUrl + "invoiceedit/GetData?action=purchase&id=1",
    });
    ...
    //   allowPdfExport: true,
      ...
      cellSelected: (args) => {
        ...
        //const url='/api/actions/invoicecreate?' + "_id=" + _id + "&fullname=" + fullname;
        //receiptvoucher
        if(keys.length == 5 && keys[3] != 'undefined' && keys[3] != '' && typeof keys[3] != 'undefined')
        {
          var voucherType = args.data.Type;
          ...
          $.ajax({
            ...
            dataType: "json",
            cache: true,
            data: { language: "English", invoice: keys[2], ClientID: keys[0], invoiceSchemaID: keys[1], transcation: keys[4], invoiceNumber: args.data.invoiceNumber, docModel: voucherType, mode: "invoice", docID: keys[3], id: keys[0] },
            success: function (fixtures) {
                var html = fixtures["html"];
                // var paid = fixtures["paid"];
                $("#panel1").html(html);
          ...
          $.ajax({
            ...
            dataType: "json",
            cache: true,
            data: { language: "English", invoice: keys[2], ClientID: keys[0], invoiceSchemaID: keys[1], transcation: keys[4], invoiceNumber: args.data.invoiceNumber, type: args.data.Type, mode: "invoice", docID: "", id: keys[0] },
            success: function (fixtures) {
                var html = fixtures["html"];
                // var paid = fixtures["paid"];
                $("#panel1").html(html);

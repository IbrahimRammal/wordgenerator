$(document).ready(function () {
  ...
  $.ajax({
      url: "/api/invoice/invoiceedit/GetData?action=unofficial&id=2",
      type: "POST",
      dataType: "json",
      cache: true,
      //data: { },
  ...
  var data = new ej.data.DataManager({
      ...
      insertUrl: hostUrl + "invoiceedit/GetData?action=unofficial&id=2",
      updateUrl: hostUrl + "invoiceedit/GetData?action=unofficial&id=2",
      removeUrl: hostUrl + "invoiceedit/GetData?action=unofficial&id=2",
  });
  ...
  //   allowPdfExport: true,
    ...
    cellSelected: (args) => {
      ...
      $.ajax({
        ...
        data: { language: "English", docModel: args.data.category, docID: args.data.docid,currency: args.data.currency, total: args.data.total, id: args.data.paymentid, mode: "invoice", voucher: voucherID, transcation: transcationID },
        success: function (fixtures) {
        ...
        //const url='/api/actions/invoicecreate?' + "_id=" + _id + "&fullname=" + fullname;
        //receiptvoucher
        if(keys.length == 5 && keys[3] != 'undefined' && typeof keys[3] != 'undefined' && keys[3] !="")
        {
          ...
          $.ajax({
            ...
            dataType: "json",
            cache: true,
            data: { language: "English", invoice: keys[2], ClientID: keys[0], invoiceSchemaID: keys[1],currency: args.data.currency, total: args.data.total, transcation: keys[4], invoiceNumber: args.data.invoiceNumber, docModel: "UnofficialReceiptVoucher", mode: "invoice", docID: keys[3], id: keys[0] },
            success: function (fixtures) {
                var html = fixtures["html"];
                // var paid = fixtures["paid"];
                $("#panel1").html(html);
          ...
          $.ajax({
            ...
            dataType: "json",
            cache: true,
            data: { language: "English", invoice: keys[2], ClientID: keys[0], invoiceSchemaID: keys[1],currency: args.data.currency, total: args.data.total, transcation: keys[4], invoiceNumber: args.data.invoiceNumber, type: "Unofficial Receipt Voucher", mode: "invoice", docID: "", id: keys[0] },
            success: function (fixtures) {
                var html = fixtures["html"];
                // var paid = fixtures["paid"];
                $("#panel1").html(html);
